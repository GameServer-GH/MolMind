...
命令默认在**仓库根目录**执行。镜像 tag 统一为 `molmind:0.1.1`，Compose 文件为 `deploy/docker-compose.yml`。
...
```text
8.133.197.65:5001/molmind:0.1.1
```
...
```bash
docker pull ghcr.io/gameserver-gh/molmind:0.1.1
docker tag ghcr.io/gameserver-gh/molmind:0.1.1 molmind:0.1.1
mkdir -p output
docker compose -f deploy/docker-compose.yml up -d
```
...
```bash
docker pull --platform linux/amd64 ghcr.io/gameserver-gh/molmind:0.1.1
docker tag ghcr.io/gameserver-gh/molmind:0.1.1 molmind:0.1.1
```
