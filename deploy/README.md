...
命令默认在**仓库根目录**执行。镜像 tag 统一为 `molmind:0.1.0`，Compose 文件为 `deploy/docker-compose.yml`。
...
```text
8.133.197.65:5001/molmind:0.1.0
```
...
`--platform linux/amd64`：当前交付镜像以 amd64 为主；Apple Silicon 通过模拟运行即可。
...
```bash
docker pull --platform linux/amd64 ghcr.io/gameserver-gh/molmind:0.1.0
docker tag ghcr.io/gameserver-gh/molmind:0.1.0 molmind:0.1.0
```
