...
from plugins.molmind_core.scientific.pipeline.config_loader import ROOT, AppConfig
...
def score_tox(
    ...
    *,
    ...
    # Facade-frozen (v2) and live/baked adapter (v1) EPA rows share one
    # conservative max-risk channel. Never sum both paths for R_tox.
    epa_by_identity: dict[str, EvidenceHit] = {}
    for hit in other_tox:
        if not str(hit.adapter_id or "").startswith("epa_ctx_tox_"):
            continue
        identity = str((hit.payload or {}).get("dtxsid") or hit.evidence_id)
        current = epa_by_identity.get(identity)
        if current is None or float(hit.score or 0.0) > float(current.score or 0.0):
            epa_by_identity[identity] = hit
    epa_hits = list(epa_by_identity.values())
    if epa_hits:
        active_count = sum(
