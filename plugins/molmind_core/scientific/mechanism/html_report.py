...
from plugins.molmind_core.scientific.evidence_gateway.contract import canonical_status


_ROOT = Path(__file__).resolve().parents[4]
_CURATED_MECHANISM_PATH = (
    _ROOT / "data" / "reference" / "candidate_mechanism_hypotheses_v1.json"
)


@lru_cache(maxsize=1)
def _load_curated_mechanisms() -> dict[str, dict[str, Any]]:
    """Load report-only mechanism enrichments keyed by exact CAS.

    This catalog is deliberately outside the scoring pipeline. It may enrich
    exported narratives but must never change eligibility or ranking.
    """
    try:
        payload = json.loads(_CURATED_MECHANISM_PATH.read_text(encoding="utf-8"))
    ...
    if not isinstance(rows, dict):
        return {}
    return {
        str(cas).strip(): dict(row)
        for cas, row in rows.items()
        if str(cas).strip() and isinstance(row, dict)
    }


def _curated_mechanism(mol: ScoreRecord) -> dict[str, Any] | None:
    row = _load_curated_mechanisms().get(str(mol.cas or "").strip())
...
def _channel_query_zh(status: str) -> str:
    ...
    return labels.get(normalized, normalized or "状态未标注")


def _narrate_public_source(name: str, row: Any) -> str:
    if not isinstance(row, dict):
        return f"{name}：本轮未查询。"
    raw_status = str(row.get("raw_status") or row.get("status") or "not_queried")
    ...
    if status == "not_queried":
        return f"{name}：本轮未查询。"
    # Canonical status is authoritative.  A provider audit row claiming a
    # scored count cannot override annotation_only/verified_empty semantics.
    score_bearing = status == "hit" and scored > 0 and effect not in {
        "none",
        "annotation",
        "annotation_only",
    ...
    if status == "verified_empty":
        trace = f"；保留 {hits} 条查询/身份审计痕迹" if hits > 0 else ""
        return f"{name}：查询完成，无符合计分契约的记录{trace}；空结果不代表安全或无效。"
    if hits > 0:
        if "annotation" in effect or effect in {"none", "annotation_or_audit_only"}:
            return f"{name}：{hits} 条注释/身份审计记录，不计分、不抬排名。"
        return (
...
def _public_source_summary(source_audit: dict[str, Any]) -> str:
    """Compact provider-specific summary without repeating generic prose."""
    items = [
        _narrate_public_source("ChEMBL", source_audit.get("chembl")),
        _narrate_public_source("PubChem", source_audit.get("pubchem")),
        _narrate_public_source("BindingDB", source_audit.get("bindingdb")),
    ]
    return "公共库检索：" + " ".join(items)
...
def _experiment_plan_html(
    mol: ScoreRecord,
    pathway_id: str,
    curated: dict[str, Any] | None = None,
) -> str:
    items = ["严格沿用统一双终点方案；候选特异结果不得脱离活力读出解释"]
    ...
    if curated:
        validation = str(curated.get("validation") or "").strip()
        if validation:
            items.append(validation)
    elif pathway_id == "UNRESOLVED":
    ...
    return f"""
      <section class="plan-box">
        <div class="formula-label">候选特异验证与风险控制</div>
...
def _source_label(url: str) -> str:
    ...
    if "nature.com/articles/" in url:
        return "Scientific Reports 原始研究"
...
def _curated_sources_html(curated: dict[str, Any]) -> str:
    sources = [str(x).strip() for x in curated.get("sources") or [] if str(x).strip()]
    if not sources:
        return ""
    def tier(url: str) -> str:
        if "pubmed.ncbi.nlm.nih.gov" in url or "nature.com/articles/" in url:
            return "原始文献"
        if "pubchem.ncbi.nlm.nih.gov" in url:
            return "公共数据库"
        return "供应商/二级资料"
    ...
    links = " · ".join(
        f'<span class="source-tier">[{_e(tier(url))}]</span> '
        f'<a href="{_e(url)}">{_e(_source_label(url))}</a>'
        for url in sources
    )
    return f'<p class="mechanism-sources"><b>资料：</b>{links}</p>'


def _identity_review_required(mol: ScoreRecord) -> bool:
    epa = mol.epa_audit or {}
    query = canonical_status(
        str(epa.get("raw_status") or epa.get("query_status") or "not_queried")
    )
    mapping = str(epa.get("mapping_status") or "")
    return bool(
        mol.identity_status == "identity_review_required"
        or query == "identity_review_required"
        or "structure_audit" in mapping
    )


def _shared_protocol_html(
    ...
    *,
    ...
    endpoint = next(
        (str(m.viability_endpoint).strip() for m in top if m.viability_endpoint),
        "CCK-8",
    )
    return f"""
    <article class="protocol-card">
      <header class="protocol-head">
        <div>
          <div class="eyebrow">SHARED DECISION PROTOCOL · 全候选共用</div>
          <h2>HepG2-FFA 双终点主筛与机制升级规则</h2>
        </div>
        <span class="badge claim">方案层 · 不改计算排名</span>
      </header>
      <p class="protocol-lead">本页把所有候选放在同一判读框架下：只有脂质蓄积下降且细胞活力未明显受损，才进入候选级机制验证。FFA 配比、处理时长和脂质命中阈值以组委会最终 SOP 为准，不把项目建议写成官方标准。</p>

      <section class="protocol-grid">
        <div class="protocol-panel">
          <h3>最小主筛分组</h3>
          <ol>
            <li>正常/载体对照：建立基础脂质与活力水平。</li>
            <li>FFA 模型/载体组：确认脂质蓄积模型成立。</li>
            <li>FFA + 候选物：固定 {concentration:g} μM 初筛。</li>
            <li>无细胞化合物背景孔：排除颜色、荧光或 {endpoint} 化学干扰。</li>
            <li>阳性对照仅在组委会 SOP 指定时采用，避免自定义标准。</li>
          </ol>
        </div>
        <div class="protocol-panel">
          <h3>平行读出与 QC</h3>
          <ul>
            <li>脂质读出与 {endpoint} 使用同一浓度/时长的平行孔或平行板。</li>
            <li>脂质信号按细胞数、核计数或总蛋白归一化，避免细胞减少造成假降脂。</li>
            <li>记录沉淀、细胞形态、DMSO 终浓度、板位和批次；必要时补 LDH/ATP。</li>
            <li>至少保留原始孔级读数、代表性图像和异常孔说明。</li>
          </ul>
        </div>
      </section>

      <section class="decision-grid">
        <div class="decision hit"><b>进入机制验证</b><span>脂质下降 + 活力 ≥{viability_pct:.0f}% + 无明显读出干扰</span></div>
        <div class="decision review"><b>复核</b><span>脂质下降但活力或归一化边界不清，先补正交活力/细胞数</span></div>
        <div class="decision reject"><b>不认定命中</b><span>脂质下降伴活力明显下降、沉淀或化学干扰</span></div>
      </section>

      <section class="upgrade-box">
        <h3>命中后的证据升级顺序</h3>
        <div class="upgrade-flow">
          <span>固定浓度双终点</span><i>→</i><span>1/3/10/30 μM 剂量反应</span><i>→</i><span>TG/胆固醇正交定量</span><i>→</i><span>候选特异标志物</span><i>→</i><span>拮抗剂/敲低/救援</span>
        </div>
        <p>EC50、CC50 与 SI 只在剂量反应后估计；单次 10 μM 结果不得外推为治疗窗或安全性结论。</p>
      </section>
    </article>
    """


def _lineage_html(lineage: dict[str, str]) -> str:
    fields = (
        ("Run ID", "run_id"),
    ...
    rows = "".join(
        f"<dt>{_e(label)}</dt><dd><code>{_e(lineage.get(key) or 'not_recorded')}</code></dd>"
        for label, key in fields
    )
    return (
        '<section class="lineage-box">'
        '<div class="formula-label">Reproducibility lineage · 复现标识</div>'
        f"<dl>{rows}</dl>"
        "</section>"
    )
...
def _candidate_html(rank: int, mol: ScoreRecord) -> str:
    ...
    curated = _curated_mechanism(mol)
    ...
    if curated:
        pathway_name = str(curated.get("label") or "候选级机制假说")
        targets = ", ".join(str(x) for x in curated.get("targets") or []) or "待定"
        mechanism_support = str(curated.get("basis") or "公开药理资料支持的候选级线索")
        mechanism_hypothesis = str(
            curated.get("hypothesis") or "候选特异机制仍需在 HepG2-FFA 中验证。"
        )
        mechanism_validation = str(
            curated.get("validation") or "先做统一双终点，再做候选特异验证。"
        )
        mechanism_falsification = str(
            curated.get("falsification") or "若双终点不成立，则不升级机制主张。"
        )
        evidence_level = str(curated.get("evidence_level") or "L5")
    elif pathway_id == "UNRESOLVED":
        mechanism_support = (
            f"{support}（最近参照 {mol.novelty_nearest_reference or '无'}，"
            f"sim={mol.novelty_max_similarity:.2f}），不编造靶点。"
        )
        mechanism_hypothesis = "目前缺少候选级通路证据；仅保留可证伪的无偏机制假说。"
        mechanism_validation = "先做统一双终点；命中后再做无偏机制解析。"
        mechanism_falsification = "若活力不达标或脂质下降不可重复，则不认定候选级机制。"
        evidence_level = "L5"
    else:
        mechanism_support = support
        mechanism_hypothesis = "结构相似性支持该通路作为候选级工作假说。"
        mechanism_validation = "先做统一双终点；命中后再做通路标志物/救援验证。"
        mechanism_falsification = "若通路标志物与表型不一致，则回退为未解析机制。"
        evidence_level = "L4"
    risk_class = (
        "risk"
    ...
    identity_badge = (
        '<span class="badge identity-review">EPA 身份映射待结构复核</span>'
        if _identity_review_required(mol)
        else ""
    )
    return f"""
    <article class="candidate-card">
      <header class="candidate-head">
        <div>
          ...
          {f'<div class="compound-name">{_e(curated.get("name"))}</div>' if curated and curated.get("name") else ''}
      ...
      <section class="mechanism-box">
        <div><span class="eyebrow">可检验机制假说</span><h3>{_e(pathway_name)} <span class="evidence-level">{_e(evidence_level)}</span></h3></div>
        <p><b>候选级假说：</b>{_e(mechanism_hypothesis)}</p>
        <p><b>靶点/通路：</b>{_e(targets)}　<b>依据与边界：</b>{_e(mechanism_support)}</p>
        <p><b>关键验证：</b>{_e(mechanism_validation)}</p>
        <p><b>推翻条件：</b>{_e(mechanism_falsification)}</p>
        {_curated_sources_html(curated) if curated else ''}
      </section>

      {_experiment_plan_html(mol, pathway_id, curated)}
    </article>
    """
...
<style>
  ...
  * {{ box-sizing:border-box; }}
  ...
  .lineage-box {{ margin-top:7mm; padding:7px 10px; border:1px solid #e5e7eb; border-radius:10px; background:#fafbfc; }}
  .lineage-box .formula-label {{ margin:0 0 4px; }}
  .lineage-box dl {{ display:grid; grid-template-columns:26mm 1fr; gap:2px 7px; }}
  .lineage-box dt {{ color:#667085; font-size:7px; }}
  .lineage-box dd {{ margin:0; color:#344054; font-size:7px; overflow-wrap:anywhere; word-break:break-all; }}
  ...
  .compound-name {{ margin-top:2px; color:#344054; font-size:9px; font-weight:800; max-width:110mm; overflow-wrap:anywhere; }}
  .eyebrow {{ color:var(--primary); font-size:9px; font-weight:800; letter-spacing:.12em; }}
  .identity {{ color:var(--muted); font-size:9px; }}
  .badges {{ display:flex; gap:4px; align-items:flex-start; flex-wrap:nowrap; justify-content:flex-end; }}
  .badge {{ padding:3px 6px; border-radius:999px; border:0; font-size:7px; font-weight:900; letter-spacing:.01em; white-space:nowrap; }}
  .eligible {{ color:#005338; background:#dff7eb; }} .neutral {{ color:#255f7c; background:#e8f3f8; }} .risk {{ color:#92400e; background:#fff1cc; }}
  .identity-review {{ color:#8a3b00; background:#ffedd5; }}
  .claim {{ color:#5b21b6; background:#f3e8ff; max-width:none; white-space:nowrap; text-align:right; line-height:1.2; }}
  ...
  .evidence-level {{ display:inline-block; margin-left:5px; padding:2px 6px; border-radius:999px; color:#5b21b6; background:#f3e8ff; font-size:8px; vertical-align:middle; }}
  .mechanism-sources {{ margin:4px 0 0 !important; font-size:8px !important; }}
  .mechanism-sources a {{ color:var(--primary); text-decoration:underline; }}
  .source-tier {{ color:#667085; font-weight:800; }}
  .protocol-card {{ padding:11px 13px; margin:0 0 8mm; border:1px solid #d9d7ff; border-radius:14px; background:linear-gradient(135deg,#fbfaff,#f5f7ff); break-inside:avoid-page; page-break-inside:avoid; page-break-after:always; }}
  .protocol-head {{ display:flex; justify-content:space-between; gap:10px; align-items:flex-start; border-bottom:1px solid #dedcff; padding-bottom:7px; }}
  .protocol-head h2 {{ margin:1px 0 0; color:var(--primary-dark); font-size:20px; font-weight:900; }}
  .protocol-lead {{ margin:8px 0; color:#435047; font-size:10px; line-height:1.55; }}
  .protocol-grid {{ display:grid; grid-template-columns:1fr 1fr; gap:8px; }}
  .protocol-panel {{ padding:8px 10px; border-radius:10px; background:#fff; border:1px solid #e7e8f3; }}
  .protocol-panel h3,.upgrade-box h3 {{ margin:0 0 4px; color:#191c1e; font-size:11px; font-weight:900; }}
  .protocol-panel ol,.protocol-panel ul {{ margin:0; padding-left:16px; display:grid; gap:3px; }}
  .protocol-panel li {{ color:#435047; font-size:9px; line-height:1.38; }}
  .decision-grid {{ display:grid; grid-template-columns:repeat(3,1fr); gap:7px; margin-top:8px; }}
  .decision {{ display:flex; flex-direction:column; gap:2px; padding:8px 9px; border-radius:9px; font-size:8px; line-height:1.35; }}
  .decision b {{ font-size:9px; }}
  .decision.hit {{ background:#e8f8f1; color:#065f46; }} .decision.review {{ background:#fff7df; color:#92400e; }} .decision.reject {{ background:#ffefef; color:#991b1b; }}
  .upgrade-box {{ margin-top:8px; padding:8px 10px; border-radius:10px; background:#fff; border:1px solid #e7e8f3; }}
  .upgrade-flow {{ display:flex; flex-wrap:wrap; align-items:center; gap:4px; color:#344054; font-size:8px; font-weight:800; }}
  .upgrade-flow span {{ padding:4px 6px; border-radius:6px; background:#f2f1ff; }}
  .upgrade-flow i {{ color:var(--primary); font-style:normal; }}
  .upgrade-box p {{ margin:5px 0 0; color:#667085; font-size:8px; }}
...
<main class="report">
  ...
  <section class="content">{_shared_protocol_html(top, concentration=concentration, viability_pct=viability_pct)}{candidates}</section>
