...
from plugins.molmind_core.scientific.critic import run_evidence_bound_llm_critic, rule_critic, summarize_critic_actions
...
from plugins.molmind_core.scientific.evidence_facade.hepg2_ffa_resources import resource_registry_runtime_payload
...
from plugins.molmind_core.scientific.pipeline.config_loader import ROOT, AppConfig, load_config
from plugins.molmind_core.scientific.pipeline.diagnostics import compute_diagnostics
...
from plugins.molmind_core.scientific.pipeline.run_log import LogSink, RunLogCollector
from plugins.molmind_core.scientific.pipeline.run_identity import deterministic_run_id, selection_sha256
from plugins.molmind_core.scientific.ranker import (
...
class PipelineResult:
    ...
    # In-memory identity index for read-only follow-up tools such as
    # query_evidence.  It is deliberately separate from ScoreRecord so an
    # enrichment query can recover the original structure without mutating or
    # re-running the frozen ranking result.
...
def _screen_sdf_inner(
    ...
    *,
    ...
    live_zh = "已请求（不进入同轮排名）" if allow_live else "未请求"
    live_en = "requested_non_scoring" if allow_live else "not_requested"
    log.emit(
        "INFO",
        (
            f"启动筛选：{mode_label}，使用快照={snap_zh}，显式联网授权={live_zh}，"
            f"config_hash={config.config_hash}，输入文件={source_filename or input_path.name}"
        ),
        (
            f"Screen started: {mode_label}, use_snapshot={snap_en}, "
            f"live_enrichment={live_en}, same_run_live_scoring=blocked, "
    ...
    # 短名单 Top-M：身份冲突检查 → 冻结证据重放 → 统一 eligibility 重跑。
    #
    # ``allow_live`` is an enrichment authorization, not permission for a
    # transport response to alter the same Run.  Ranking therefore always
    # reads the frozen/local facade here.  A caller that requested live access
    # must use query_evidence (read-only audit/cache) or bake-evidence, then
    # start a new offline Run whose config_hash includes the new snapshot.
    ...
    if deep_m > 0 and eligible:
        ...
        live_enrichment_requested = bool(config.allow_live_evidence)
        ...
        log.emit(
            ...
            (
                ...
                "冻结 snapshot/local 补洞→统一资格重跑；"
                f"模式={config.mode}；适配器={adapter_names}；"
                f"EPA阶段={epa_stage}（ranking_effect={epa_effect}）；"
                f"HTTP超时={timeout_sec}s；同轮 live 入榜=禁止；"
                f"显式 enrichment 请求={'是' if live_enrichment_requested else '否'}"
            ),
            (
                f"Start Top-M evidence orchestration: window={shortlist_n} "
                f"(deep_query_top_m={deep_m}); steps=identity_check→"
                "frozen_snapshot/local_fill→re-eligibility; "
                f"mode={config.mode}; adapters={adapter_names}; "
                f"epa_stage={epa_stage} (ranking_effect={epa_effect}); "
                f"http_timeout={timeout_sec}s; same_run_live_scoring=blocked; "
                f"explicit_enrichment_requested={str(live_enrichment_requested).lower()}"
            ),
            progress=62,
        )
        ...
        for mol in eligible:
            ...
            # Step 1–2: replay only frozen/local evidence. Identity status is
            # re-derived inside score_molecule from query_audit / PubChem
            # multi-CID. Live transport never enters this scoring object.
            evidence = facade.query(
                inchikey=record.inchikey,
                cas=record.cas,
                smiles=record.smiles,
        ...
        log.emit(
            ...
            (
                ...
                f"explicit_enrichment_requested={'yes' if live_enrichment_requested else 'no'}"
            ),
            (
...
def run_pipeline(
    ...
    *,
    ...
    from plugins.molmind_core.scientific.pipeline.export import export_mechanism_graph_json
    ...
    from plugins.molmind_core.scientific.pipeline.export import export_hepg2_ffa_resources_json

    export_hepg2_ffa_resources_json(result.hepg2_ffa_resources, resource_path)
    if write_mechanism and result.top_molecules:
        from plugins.molmind_core.scientific.mechanism import render_mechanism_markdown
    ...
    from plugins.molmind_core.scientific.pipeline.manifest import write_run_manifest
