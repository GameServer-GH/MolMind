...
from plugins.molmind_core.scientific.evidence_facade.mechanism_graph import MechanismGraph, serialize_mechanism_graphs
...
def rows_from_top(
    ...
    *,
    ...
    for rank, mol in enumerate(molecules, start=1):
        heads = mol.tox_heads
        source_audit = mol.evidence_source_audit or {}

        def _source_status(provider: str) -> tuple[str, str]:
            audit = source_audit.get(provider) or {}
            raw = str(audit.get("raw_status") or audit.get("status") or "not_queried")
            return canonical_status(raw), raw

        chembl_status, chembl_raw_status = _source_status("chembl")
        pubchem_status, pubchem_raw_status = _source_status("pubchem")
        bindingdb_status, bindingdb_raw_status = _source_status("bindingdb")
        epa_raw_query_status = str(
            mol.epa_audit.get("raw_status")
            or mol.epa_audit.get("query_status")
            or "not_queried"
        )
        epa_query_status = canonical_status(epa_raw_query_status)
        epa_mapping_status = str(
            mol.epa_audit.get("mapping_status", "audit_missing")
        )
        identity_review_required = bool(
            epa_query_status == "identity_review_required"
            or "structure_audit" in epa_mapping_status
            or mol.identity_status == "identity_review_required"
        )
        identity_review_reason = (
            "EPA mapping requires structure-level confirmation before evidence reuse"
        ...
        rows.append(
            {
                ...
                "run_id": run_id,
                "submission_schema_version": "molmind-nomination-v2",
                "submission_scope": "pre_wet_lab_computational_nomination",
                "evidence_limitations": (
                    "no_same_condition_lipid_or_CCK8_measurements;"
                    "proxy_priority_not_verified_efficacy_or_safety"
                ),
                ...
                "identity_review_reason": identity_review_reason,
                "epa_stage": mol.epa_audit.get("stage", 0),
                "epa_status": mol.epa_audit.get("status", "disabled"),
                "epa_query_status": epa_query_status,
                "epa_raw_query_status": epa_raw_query_status,
                "epa_mapping_status": epa_mapping_status,
                ...
                "chembl_query_status": chembl_status,
                "chembl_raw_status": chembl_raw_status,
                "pubchem_query_status": pubchem_status,
                "pubchem_raw_status": pubchem_raw_status,
                "bindingdb_query_status": bindingdb_status,
                "bindingdb_raw_status": bindingdb_raw_status,
            }
        )
    return rows
        ...
        for molecule in molecules:
            ...
            for hit in hits:
                fh.write(json.dumps({
                    ...
                    "query_status": canonical_status(hit.query_status, hit),
                    "raw_status": hit.raw_status or hit.query_status,
                    "score": hit.score,
