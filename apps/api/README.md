...
- 项目版本以根目录 `pyproject.toml` 的 `[project].version` 为唯一源码来源（当前 **0.2.2**）。
...
- Agent：`/api/agent/sessions` · message stream · events（`after_seq`）· settings / catalog
- 会话载荷含 `active_run` / `revision`；活动写冲突返回 `409 session_busy`
- Swagger UI：`GET /docs`
